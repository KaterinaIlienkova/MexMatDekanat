...
class WumpusWorld:
    ...
    def a_star(self):
        ...
        #Функція heapq.heappush використовується для додавання елементів до списку, який підтримує структуру мін-кучі (min-heap).
        # Це спеціальна структура даних, яка дозволяє ефективно отримувати елемент з найменшим значенням.
        heapq.heappush(open_list, (f_score[self.agent], self.agent)) #використовуємо min-heap для зберігання
        # відкритих кімнат (open_list). Кожен елемент у черзі — це кортеж, де перший елемент є
        # евристичною оцінкою (f_score), а другий — кімната. Це дає змогу швидко вибрати кімнату з
        # найменшим значенням f_score

                #Основний цикл:

        #Вибирається кімната з найменшим значенням f_score (найкраща оцінка).
        #Якщо ця кімната є кімнатою з вампусом, шлях знайдено і ми відновлюємо його.
        #Якщо кімната не є цільовою, то перевіряються її сусіди (для кожної сусідньої кімнати обчислюється нове значення g_score).
        #Якщо цей новий шлях до сусідньої кімнати кращий, ніж попередній, оновлюються значення для цієї кімнати (її батьківська кімната і нові оцінки).

        #Пошук завершується, коли агент доходить до кімнати з вампусом або якщо не знайдено жодного шляху.
